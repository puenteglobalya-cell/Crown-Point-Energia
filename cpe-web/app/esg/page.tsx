      ...
      >
        <div className="container">
          ...
          <h1 style={{ marginTop: 14 }}>
            <span className="lang-es" aria-hidden={lang !== 'es'} dangerouslySetInnerHTML={{ __html: cmsLineBreaks(f['page.esg.h1'] || 'Operar responsablemente.<br/>Reportar con claridad.') }} />
            <span className="lang-en" aria-hidden={lang !== 'en'} dangerouslySetInnerHTML={{ __html: cmsLineBreaks(fe['page.esg.h1'] || 'Operate responsibly.<br/>Report with clarity.') }} />
          </h1>
          <p>
            <span className="lang-es" aria-hidden={lang !== 'es'}>{f['page.esg.lede'] || 'Nuestra estrategia ESG integra la responsabilidad ambiental, el compromiso social y la gobernanza robusta como pilares de creación de valor a largo plazo.'}</span>
            <span className="lang-en" aria-hidden={lang !== 'en'}>{fe['page.esg.lede'] || 'Our ESG strategy integrates environmental responsibility, social commitment and robust governance as pillars of long-term value creation.'}</span>
          </p>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--s-12)', alignItems: 'start' }}>
            ...
            <div>
              ...
              <h2 style={{ fontSize: 28, fontFamily: 'var(--font-display)', fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 'var(--s-6)' }}>
                <span className="lang-es" aria-hidden={lang !== 'es'}>Gobierno corporativo</span>
                <span className="lang-en" aria-hidden={lang !== 'en'}>Corporate governance</span>
              </h2>
              <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: 'var(--s-2)' }}>
                {politicas.map(doc => {
                  ...
                  <li key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--rule)', fontSize: 14 }}>
                    ...
                    >
                      <span className="lang-es" aria-hidden={lang !== 'es'}>{doc.titulo_es}</span>
                      <span className="lang-en" aria-hidden={lang !== 'en'}>{doc.titulo_en || doc.titulo_es}</span>
                    </a>
                ...
                {[
                  ...
                  <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--rule)', fontSize: 14 }}>
                    ...
                    <Link href={doc.href} style={{ color: 'var(--fg)', textDecoration: 'none', flex: 1 }}>
                      <span className="lang-es" aria-hidden={lang !== 'es'}>{doc.es}</span>
                      <span className="lang-en" aria-hidden={lang !== 'en'}>{doc.en}</span>
                    </Link>
                    <span style={{ fontSize: 11, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.1em', flexShrink: 0 }}>
                      {doc.isLink ? <><span className="lang-es" aria-hidden={lang !== 'es'}>enlace</span><span className="lang-en" aria-hidden={lang !== 'en'}>link</span></> : 'PDF'}
                    </span>
                  </li>
                ))}
              </ul>
              <p style={{ fontSize: 12, color: 'var(--fg-muted)', marginTop: 'var(--s-4)', fontStyle: 'italic' }}>
                <span className="lang-es" aria-hidden={lang !== 'es'}>Los documentos se descargan desde la Biblioteca de inversores.</span>
                <span className="lang-en" aria-hidden={lang !== 'en'}>Documents are available in the Investor library section.</span>
              </p>
            </div>
            ...
            <div style={{ background: 'var(--cp-navy-darker)', borderRadius: 'var(--r-xl)', padding: 'var(--s-10)', color: '#fff' }}>
              ...
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.72)', lineHeight: 1.65, marginBottom: 'var(--s-6)' }}>
                <span className="lang-es" aria-hidden={lang !== 'es'}>Canal confidencial para reportar conductas contrarias al Código de Ética. Las denuncias pueden realizarse de forma anónima. Todas son tratadas con absoluta reserva.</span>
                <span className="lang-en" aria-hidden={lang !== 'en'}>Confidential channel to report conduct contrary to the Code of Ethics. Reports may be submitted anonymously. All are handled with complete confidentiality.</span>
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s-3)' }}>
                <Link href="/denuncias" style={{
                  ...
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Hacer una denuncia →</span><span className="lang-en" aria-hidden={lang !== 'en'}>File a report →</span>
                </Link>
