...
// producción, 0.5 es mitad y mitad. La concesión se busca por yacimiento_id
// (no por nombre — comparar texto rompe en silencio ante cualquier
// diferencia de mayúsculas/espacios entre yacimiento y concesión).
type ItemReparto = { yacimientoId: number; yacimiento: string; pozos: number }
...
export async function POST(req: NextRequest) {
  ...
  const [{ data: concesiones }, { data: pozosTipo }, { data: curvas }] = await Promise.all([
    db.from('concesiones').select('id, nombre, yacimiento_id'),
  ...
  const bblPorYacimiento = new Map<number, number>()
  for (const c of curvas ?? []) {
    const pt = (pozosTipo ?? []).find(p => p.id === c.pozo_tipo_id)
    if (!pt?.yacimiento_id) continue
    bblPorYacimiento.set(pt.yacimiento_id, (bblPorYacimiento.get(pt.yacimiento_id) ?? 0) + Number(c.bbl_petroleo ?? 0))
  }
  const totalBbl = [...bblPorYacimiento.values()].reduce((a, b) => a + b, 0)
  const hayProduccion = totalBbl > 0 && repartoIn.every(r => bblPorYacimiento.has(r.yacimientoId))
  ...
  for (const item of repartoIn) {
    ...
    if (pozos === 0) continue
    const conc = (concesiones ?? []).find(c => c.yacimiento_id === item.yacimientoId)
    if (!conc) { errores.push(`No existe una concesión con el yacimiento "${item.yacimiento}" — cargala primero`); continue }

    const pctPozos = pozos / totalPozos
    const bbl = bblPorYacimiento.get(item.yacimientoId) ?? 0
