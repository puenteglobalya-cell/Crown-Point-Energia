...
export const ENTITIES: EntityConfig[] = [
  ...
  },
  {
    ...
    helpText: 'precio = referencia × (1 − DDE%) / divisor − descuento adicional − tarifa de almacenamiento (USD/m3/día × días, convertido a USD/bbl). DDE% es fijo (campo "DDE %") salvo que completes los 4 campos de tramo por Brent — ahí se calcula solo, lineal entre el mínimo y el máximo, y el campo "DDE %" fijo se ignora.',
    fields: [
      { name: 'yacimiento_id', label: 'Yacimiento', type: 'select', optionsFrom: 'yacimientos', required: true },
      ...
      { name: 'dde_pct', label: 'DDE % fijo (vacío si usás tramo por Brent)', type: 'number', step: '0.01' },
      { name: 'dde_brent_min', label: 'Tramo: Brent mínimo (ej. 65)', type: 'number', step: '0.01' },
      { name: 'dde_pct_min', label: 'Tramo: DDE % en el mínimo (ej. 0)', type: 'number', step: '0.01' },
      { name: 'dde_brent_max', label: 'Tramo: Brent máximo (ej. 80)', type: 'number', step: '0.01' },
      { name: 'dde_pct_max', label: 'Tramo: DDE % en el máximo (ej. 8)', type: 'number', step: '0.01' },
