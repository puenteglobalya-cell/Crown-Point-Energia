...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    // Supabase detects the #access_token hash and sets up a recovery session
    const supabase = createSupabaseBrowserClient()
    supabase.auth.onAuthStateChange((event) => {
      if (event === 'PASSWORD_RECOVERY') setReady(true)
    })
    // Also check if session already exists (e.g. navigated here after hash was consumed)
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 400, background: 'var(--surface)', border: '1px solid var(--rule)', borderRadius: 'var(--r-lg)', padding: '40px 36px' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--fg-soft)', marginBottom: 28 }}>
          Elegí una contraseña para tu cuenta de admin.
        </p>

        {success ? (
          <div style={{ background: 'color-mix(in oklab, var(--cp-green) 14%, var(--surface))', border: '1px solid var(--cp-green)', padding: 16, borderRadius: 'var(--r-md)', fontSize: 14 }}>
