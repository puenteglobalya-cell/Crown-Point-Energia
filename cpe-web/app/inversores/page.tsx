      ...
      >
        <div className="container">
          <div className="crumbs">
            ...
            <span>/</span>
            <span><span className="lang-es" aria-hidden={lang !== 'es'}>Invertir</span><span className="lang-en" aria-hidden={lang !== 'en'}>Invest</span></span>
          </div>
          <span className="eyebrow"><span className="lang-es" aria-hidden={lang !== 'es'}>Resumen del inversor</span><span className="lang-en" aria-hidden={lang !== 'en'}>Investor overview</span></span>
          <h1 style={{ marginTop: 14 }}>
            <span className="lang-es" aria-hidden={lang !== 'es'} dangerouslySetInnerHTML={{ __html: cmsLineBreaks(f['page.inversores.h1'] || 'Una historia sólida<br/>de creación de valor.') }} />
            <span className="lang-en" aria-hidden={lang !== 'en'} dangerouslySetInnerHTML={{ __html: cmsLineBreaks(fe['page.inversores.h1'] || 'A solid story<br/>of value creation.') }} />
          </h1>
          <p>
            <span className="lang-es" aria-hidden={lang !== 'es'}>{f['page.inversores.lede'] || 'Crown Point Energía S.A. es una empresa dedicada al petróleo y gas con cobertura internacional que opera en el mercado argentino.'}</span>
            <span className="lang-en" aria-hidden={lang !== 'en'}>{fe['page.inversores.lede'] || 'Crown Point Energy Inc. is a public company incorporated in Alberta, Canada and listed on the TSX Venture Exchange (TSXV) under the symbol "CWV". Crown Point operates in Argentina through its wholly owned subsidiary Crown Point Energía S.A. headquartered in Buenos Aires.'}</span>
          </p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="two-col">
            <aside className="left-rail">
              ...
              <nav>
                <a href="#porque" className="active"><span className="lang-es" aria-hidden={lang !== 'es'}>¿Por qué Crown Point?</span><span className="lang-en" aria-hidden={lang !== 'en'}>Why Crown Point?</span></a>
                <a href="#calificacion"><span className="lang-es" aria-hidden={lang !== 'es'}>Calificación crediticia</span><span className="lang-en" aria-hidden={lang !== 'en'}>Credit rating</span></a>
                {(showFinancieros || showCpesa) && <a href="#financieros"><span className="lang-es" aria-hidden={lang !== 'es'}>Estados financieros</span><span className="lang-en" aria-hidden={lang !== 'en'}>Financial statements</span></a>}
                {showAgm && <a href="#agm"><span className="lang-es" aria-hidden={lang !== 'es'}>AGM / Asambleas CPI</span><span className="lang-en" aria-hidden={lang !== 'en'}>AGM / CPI meetings</span></a>}
                {showEstma && <a href="#estma"><span className="lang-es" aria-hidden={lang !== 'es'}>ESTMA</span><span className="lang-en" aria-hidden={lang !== 'en'}>ESTMA</span></a>}
                {showCnv && <a href="#hechos-cnv"><span className="lang-es" aria-hidden={lang !== 'es'}>Hechos relevantes CNV</span><span className="lang-en" aria-hidden={lang !== 'en'}>CNV disclosures</span></a>}
                {showCobertura && <a href="#cobertura"><span className="lang-es" aria-hidden={lang !== 'es'}>Cobertura de analistas</span><span className="lang-en" aria-hidden={lang !== 'en'}>Analyst coverage</span></a>}
                {showOn && <a href="#on"><span className="lang-es" aria-hidden={lang !== 'es'}>Obligaciones negociables</span><span className="lang-en" aria-hidden={lang !== 'en'}>Notes</span></a>}
                {showGobierno && <a href="#gobierno"><span className="lang-es" aria-hidden={lang !== 'es'}>Gobierno corporativo</span><span className="lang-en" aria-hidden={lang !== 'en'}>Corporate governance</span></a>}
                {showAsambleas && <a href="#asambleas"><span className="lang-es" aria-hidden={lang !== 'es'}>Asambleas de accionistas</span><span className="lang-en" aria-hidden={lang !== 'en'}>Shareholder meetings</span></a>}
                {showCalendario && <a href="#calendario"><span className="lang-es" aria-hidden={lang !== 'es'}>Calendario financiero</span><span className="lang-en" aria-hidden={lang !== 'en'}>Financial calendar</span></a>}
              </nav>
            </aside>
            <main>
              <div className="section-block" id="porque">
                <span className="eyebrow"><span className="lang-es" aria-hidden={lang !== 'es'}>Tesis de inversión</span><span className="lang-en" aria-hidden={lang !== 'en'}>Investment thesis</span></span>
                <h2 style={{ marginTop: 8 }}><span className="lang-es" aria-hidden={lang !== 'es'}>Tres motivos para mirar Crown Point.</span><span className="lang-en" aria-hidden={lang !== 'en'}>Three reasons to look at Crown Point.</span></h2>
                <div className="kpi-grid kpi-grid-3" style={{ marginTop: 'var(--s-6)' }}>
                  {[
                    ...
                    <div className="kpi" key={k.n}>
                      <span className="kpi-label">{k.n} · <span className="lang-es" aria-hidden={lang !== 'es'}>{k.labelEs}</span><span className="lang-en" aria-hidden={lang !== 'en'}>{k.labelEn}</span></span>
                      <div><span className="kpi-value num">{k.val}</span><span className="kpi-unit">{k.unit || <><span className="lang-es" aria-hidden={lang !== 'es'}>{k.unitEs}</span><span className="lang-en" aria-hidden={lang !== 'en'}>{k.unitEn}</span></>}</span></div>
                      <span className="kpi-meta"><span className="lang-es" aria-hidden={lang !== 'es'}>{k.metaEs}</span><span className="lang-en" aria-hidden={lang !== 'en'}>{k.metaEn}</span></span>
                    </div>
                  ))}
                </div>
                ...
                <p style={{ fontSize: 12, color: 'var(--fg-muted)', marginTop: 'var(--s-2)' }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Cifras gerenciales no auditadas en base a los estados contables de CPE Inc. Para detalle ver <a href="#financieros">Estados financieros</a>.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Unaudited management figures based on CPE Inc. financial statements. See <a href="#financieros">Financial statements</a> for detail.</span>
                </p>
              </div>

              <div className="section-block" id="calificacion">
                <span className="eyebrow"><span className="lang-es" aria-hidden={lang !== 'es'}>Deuda local</span><span className="lang-en" aria-hidden={lang !== 'en'}>Local debt</span></span>
                <h2 style={{ marginTop: 8 }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Calificación crediticia</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Credit rating</span>
                </h2>
                <p className="lede">
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Calificación otorgada por <a href="https://www.fixscr.com/emisor/view?type=emisor&id=4052" target="_blank" rel="noreferrer">FIX SCR</a> (afiliada de Fitch Ratings en Argentina) sobre obligaciones de largo plazo.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Rating assigned by <a href="https://www.fixscr.com/emisor/view?type=emisor&id=4052" target="_blank" rel="noreferrer">FIX SCR</a> (Fitch Ratings affiliate in Argentina) on long-term obligations.</span>
                </p>
                <style>{`
                ...
                <table className="fix-table">
                  <thead>
                    <tr>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Fecha</span><span className="lang-en" aria-hidden={lang !== 'en'}>Date</span></th>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Plazo</span><span className="lang-en" aria-hidden={lang !== 'en'}>Tenor</span></th>
                      <th>Rating</th>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Perspectiva</span><span className="lang-en" aria-hidden={lang !== 'en'}>Outlook</span></th>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Acción</span><span className="lang-en" aria-hidden={lang !== 'en'}>Action</span></th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                ...
                <p style={{ marginTop: 'var(--s-4)', fontSize: 12, color: 'var(--fg-muted)' }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Ver ficha completa en </span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Full report at </span>
              ...
              {(showFinancieros || showCpesa) && <div className="section-block" id="financieros">
                <span className="eyebrow"><span className="lang-es" aria-hidden={lang !== 'es'}>Reportes recientes</span><span className="lang-en" aria-hidden={lang !== 'en'}>Recent filings</span></span>
                <h2 style={{ marginTop: 8 }}><span className="lang-es" aria-hidden={lang !== 'es'}>Estados financieros</span><span className="lang-en" aria-hidden={lang !== 'en'}>Financial statements</span></h2>
                <p className="lede">
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Reportes auditados según IFRS (CPI, TSXV: CWV) y estados contables de CPESA ante la <a href="https://www.cnv.gob.ar" target="_blank" rel="noreferrer">CNV</a>. Disponibles también en <a href="https://www.sedarplus.ca" target="_blank" rel="noreferrer">SEDAR+</a>.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>IFRS-audited reports (CPI, TSXV: CWV) and CPESA financial statements filed with the <a href="https://www.cnv.gob.ar" target="_blank" rel="noreferrer">CNV</a>. Also available on <a href="https://www.sedarplus.ca" target="_blank" rel="noreferrer">SEDAR+</a>.</span>
                </p>
              ...
              {showAgm && <div className="section-block" id="agm">
                ...
                <h2 style={{ marginTop: 8 }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Materiales de asamblea — CPI</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>AGM materials — CPI</span>
                </h2>
                <p className="lede">
              ...
              {showEstma && <div className="section-block" id="estma">
                ...
                <h2 style={{ marginTop: 8 }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Reportes ESTMA</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>ESTMA Reports</span>
                </h2>
                <p className="lede">
              ...
              {showCnv && <div className="section-block" id="hechos-cnv">
                ...
                <h2 style={{ marginTop: 8 }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Hechos relevantes</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Material disclosures</span>
                </h2>
                <p className="lede">
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Información presentada por Crown Point Energía S.A. ante la Comisión Nacional de Valores. Actualizado automáticamente desde la <a href="https://www.cnv.gov.ar/SitioWeb/Empresas/Empresa/30709346268" target="_blank" rel="noreferrer">AIF de la CNV</a>.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Disclosures filed by Crown Point Energía S.A. with the Argentine National Securities Commission (CNV). Auto-synced from the <a href="https://www.cnv.gov.ar/SitioWeb/Empresas/Empresa/30709346268" target="_blank" rel="noreferrer">CNV AIF</a>.</span>
                </p>
                {cnvHechos.length > 0 ? (
                      ...
                      <thead>
                        <tr style={{ background: 'var(--bg-alt)', borderBottom: '1px solid var(--rule)' }}>
                          <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 600, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--fg-muted)', width: 110 }}>
                            <span className="lang-es" aria-hidden={lang !== 'es'}>Fecha</span><span className="lang-en" aria-hidden={lang !== 'en'}>Date</span>
                          </th>
                          <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 600, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--fg-muted)' }}>
                            <span className="lang-es" aria-hidden={lang !== 'es'}>Descripción</span><span className="lang-en" aria-hidden={lang !== 'en'}>Description</span>
                          </th>
                        ...
                        {cnvHechos.map((h, i) => (
                          <tr key={h.doc_id} style={{ borderTop: i > 0 ? '1px solid var(--rule)' : 'none', background: 'var(--surface)' }}>
                            <td style={{ padding: '12px 16px', color: 'var(--fg-muted)', whiteSpace: 'nowrap', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                              <span className="lang-es" aria-hidden={lang !== 'es'}>{new Date(h.fecha + 'T00:00:00').toLocaleDateString('es-AR', { day: '2-digit', month: 'short', year: 'numeric' })}</span><span className="lang-en" aria-hidden={lang !== 'en'}>{new Date(h.fecha + 'T00:00:00').toLocaleDateString('en-CA', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                            </td>
              ...
              {showCobertura && <div className="section-block" id="cobertura">
                ...
                <h2 style={{ marginTop: 8 }}><span className="lang-es" aria-hidden={lang !== 'es'}>Cobertura de analistas</span><span className="lang-en" aria-hidden={lang !== 'en'}>Analyst coverage</span></h2>
                <p className="lede"><span className="lang-es" aria-hidden={lang !== 'es'}>Brokers e instituciones financieras que mantienen cobertura activa sobre el papel.</span><span className="lang-en" aria-hidden={lang !== 'en'}>Brokers and financial institutions actively covering the stock.</span></p>
                {analysts.length > 0 ? (
                    ...
                    {analysts.map(a => (
                      <div className="analyst" key={a.id}>
                        ...
                        <span className={`ar-rec ${a.rating_es.toLowerCase().includes('compra') || a.rating_es.toLowerCase() === 'buy' ? 'buy' : 'hold'}`}>
                          <span className="lang-es" aria-hidden={lang !== 'es'}>{a.rating_es}</span>
                          <span className="lang-en" aria-hidden={lang !== 'en'}>{a.rating_en}</span>
                        </span>
              ...
              {showOn && <div className="section-block" id="on">
                <span className="eyebrow"><span className="lang-es" aria-hidden={lang !== 'es'}>Programa global</span><span className="lang-en" aria-hidden={lang !== 'en'}>Global program</span></span>
                <h2 style={{ marginTop: 8 }}><span className="lang-es" aria-hidden={lang !== 'es'}>Obligaciones negociables activas</span><span className="lang-en" aria-hidden={lang !== 'en'}>Active notes</span></h2>
                <p className="lede">
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Calificaciones otorgadas por <a href="https://www.fixscr.com/emisor/view?type=emisor&id=4052" target="_blank" rel="noreferrer">FIX SCR</a> (afiliada de Fitch Ratings en Argentina). Programa global de emisión autorizado por CNV.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Ratings assigned by <a href="https://www.fixscr.com/emisor/view?type=emisor&id=4052" target="_blank" rel="noreferrer">FIX SCR</a> (Fitch Ratings affiliate in Argentina). Global issuance program authorized by the CNV.</span>
                </p>
                <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', marginTop: 'var(--s-4)' }}>
                <table className="on-table">
                  <thead>
                    <tr>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Instrumento</span><span className="lang-en" aria-hidden={lang !== 'en'}>Instrument</span></th>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Fecha</span><span className="lang-en" aria-hidden={lang !== 'en'}>Date</span></th>
                      <th>ISIN</th>
                      <th>Rating</th>
                      <th><span className="lang-es" aria-hidden={lang !== 'es'}>Perspectiva</span><span className="lang-en" aria-hidden={lang !== 'en'}>Outlook</span></th>
              ...
              {showGobierno && <div className="section-block" id="gobierno">
                ...
                <div style={{ marginTop: 'var(--s-5)', padding: '14px 18px', border: '1px solid var(--rule)', borderRadius: 'var(--r-md)', background: 'var(--bg-alt)', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, color: 'var(--fg-muted)' }}>
                    <span className="lang-es" aria-hidden={lang !== 'es'}>Línea de denuncias</span><span className="lang-en" aria-hidden={lang !== 'en'}>Complaint hotline</span>
                  </span>
              ...
              {showAsambleas && <div className="section-block" id="asambleas">
                <span className="eyebrow"><span className="lang-es" aria-hidden={lang !== 'es'}>Accionistas</span><span className="lang-en" aria-hidden={lang !== 'en'}>Shareholders</span></span>
                <h2 style={{ marginTop: 8 }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Asambleas de accionistas</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Shareholder meetings</span>
                </h2>
                <p className="lede">
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Información sobre próximas asambleas de Crown Point Energy Inc. Los materiales de cada reunión se publican en <a href="https://www.sedarplus.ca" target="_blank" rel="noreferrer">SEDAR+</a> con al menos 21 días de anticipación.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Information on upcoming Crown Point Energy Inc. shareholder meetings. Meeting materials are filed on <a href="https://www.sedarplus.ca" target="_blank" rel="noreferrer">SEDAR+</a> at least 21 days in advance.</span>
                </p>
                <style>{`
                    ...
                    <div className="meeting-card" key={m.id}>
                      <div className="meeting-card-head">
                        <span className={`meeting-type-badge ${m.tipo}`}>
                          {m.tipo === 'agm'
                            ? <><span className="lang-es" aria-hidden={lang !== 'es'}>Asamblea General Anual</span><span className="lang-en" aria-hidden={lang !== 'en'}>Annual General Meeting</span></>
                            : <><span className="lang-es" aria-hidden={lang !== 'es'}>Asamblea Extraordinaria</span><span className="lang-en" aria-hidden={lang !== 'en'}>Extraordinary General Meeting</span></>
                          }
                        </span>
                        <span className="meeting-date-big">
                          <span className="lang-es" aria-hidden={lang !== 'es'}>{dateEs}</span>
                          <span className="lang-en" aria-hidden={lang !== 'en'}>{dateEn}</span>
                        </span>
                      </div>

                      <div className="meeting-body">
                        <div className="meeting-col">
                          <div className="meeting-col-label"><span className="lang-es" aria-hidden={lang !== 'es'}>Hora</span><span className="lang-en" aria-hidden={lang !== 'en'}>Time</span></div>
                          <div className="meeting-col-val">{m.hora_local || '—'}</div>
                          {(m.zona_es || m.zona_en) && (
                            <div className="meeting-col-sub">
                              <span className="lang-es" aria-hidden={lang !== 'es'}>{m.zona_es}</span>
                              <span className="lang-en" aria-hidden={lang !== 'en'}>{m.zona_en}</span>
                            </div>
                          )}
                        </div>
                        <div className="meeting-col">
                          <div className="meeting-col-label"><span className="lang-es" aria-hidden={lang !== 'es'}>Lugar</span><span className="lang-en" aria-hidden={lang !== 'en'}>Venue</span></div>
                          <div className="meeting-col-val">
                            <span className="lang-es" aria-hidden={lang !== 'es'}>{m.lugar_es || '—'}</span>
                            <span className="lang-en" aria-hidden={lang !== 'en'}>{m.lugar_en || '—'}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {(m.nota_es || m.nota_en) && (
                        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--rule)' }}>
                          <div className="meeting-col-label" style={{ marginBottom: 8 }}><span className="lang-es" aria-hidden={lang !== 'es'}>Agenda / Asuntos</span><span className="lang-en" aria-hidden={lang !== 'en'}>Agenda / Business</span></div>
                          <p style={{ fontSize: 14, color: 'var(--fg-soft)', lineHeight: 1.6, margin: 0 }}>
                            <span className="lang-es" aria-hidden={lang !== 'es'}>{m.nota_es}</span>
                            <span className="lang-en" aria-hidden={lang !== 'en'}>{m.nota_en}</span>
                          </p>
                        </div>
                      )}
                      ...
                      <div className="meeting-footer">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                          <span className="formato-pill">
                            {m.formato === 'virtual'
                              ? <><span className="lang-es" aria-hidden={lang !== 'es'}>Virtual</span><span className="lang-en" aria-hidden={lang !== 'en'}>Virtual</span></>
                              : m.formato === 'presencial'
                              ? <><span className="lang-es" aria-hidden={lang !== 'es'}>Presencial</span><span className="lang-en" aria-hidden={lang !== 'en'}>In-person</span></>
                              : <><span className="lang-es" aria-hidden={lang !== 'es'}>Presencial + Virtual</span><span className="lang-en" aria-hidden={lang !== 'en'}>In-person + Virtual</span></>
                            }
                          </span>
                          {recordEs && (
                            <span className="meeting-footer-note">
                              <span className="lang-es" aria-hidden={lang !== 'es'}>Fecha de registro: {recordEs}</span>
                              <span className="lang-en" aria-hidden={lang !== 'en'}>Record date: {recordEn}</span>
                            </span>
                          )}
                        </div>
                        {m.sedar_url && (
                          ...
                          >
                            <span className="lang-es" aria-hidden={lang !== 'es'}>Documentos en SEDAR+</span>
                            <span className="lang-en" aria-hidden={lang !== 'en'}>Meeting documents on SEDAR+</span>
              ...
              {showCalendario && <div className="section-block" id="calendario" style={{ borderBottom: 0 }}>
                ...
                <h2 style={{ marginTop: 8 }}>
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Calendario financiero</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Financial calendar</span>
                </h2>
                <p className="lede">
                  <span className="lang-es" aria-hidden={lang !== 'es'}>Fechas tentativas para resultados trimestrales y eventos corporativos. Sujeto a cambio — confirmación en SEDAR+ con al menos 2 semanas de anticipación.</span>
                  <span className="lang-en" aria-hidden={lang !== 'en'}>Tentative dates for quarterly results and corporate events. Subject to change — confirmation on SEDAR+ at least 2 weeks in advance.</span>
                </p>
                <style>{`
                  ...
                  {irEvents.map(ev => (
                    <div className="cal-event" key={ev.id}>
                      <span className={`cal-type-badge ${ev.tipo}`}>
                        {ev.tipo === 'results'
                          ? <><span className="lang-es" aria-hidden={lang !== 'es'}>Resultados</span><span className="lang-en" aria-hidden={lang !== 'en'}>Results</span></>
                          : ev.tipo === 'agm'
                          ? <><span className="lang-es" aria-hidden={lang !== 'es'}>Asamblea</span><span className="lang-en" aria-hidden={lang !== 'en'}>AGM</span></>
                          : ev.tipo}
                      </span>
                      <div className="cal-event-date">
                        <span className="lang-es" aria-hidden={lang !== 'es'}>{ev.fecha}</span>
                        <span className="lang-en" aria-hidden={lang !== 'en'}>{ev.fecha}</span>
                      </div>
                      <div className="cal-event-title">
                        <span className="lang-es" aria-hidden={lang !== 'es'}>{ev.titulo_es}</span>
                        <span className="lang-en" aria-hidden={lang !== 'en'}>{ev.titulo_en}</span>
                      </div>
                      <div className="cal-event-note">
                        <span className="lang-es" aria-hidden={lang !== 'es'}>{ev.nota_es}</span>
                        <span className="lang-en" aria-hidden={lang !== 'en'}>{ev.nota_en}</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <section className="invest-feature" style={{ paddingTop: 'var(--s-12)', paddingBottom: 'var(--s-12)' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 'var(--s-12)', alignItems: 'center' }} className="news-grid">
            <div>
              <span className="eyebrow" style={{ color: 'var(--cp-green)' }}><span className="lang-es" aria-hidden={lang !== 'es'}>Alertas para inversores</span><span className="lang-en" aria-hidden={lang !== 'en'}>Investor alerts</span></span>
              <h2 className="section-title" style={{ color: '#fff', marginTop: 8 }}>
                <span className="lang-es" aria-hidden={lang !== 'es'}>Recibí los comunicados<br/>antes que nadie.</span>
                <span className="lang-en" aria-hidden={lang !== 'en'}>Get our releases<br/>before anyone else.</span>
              </h2>
              <p style={{ color: 'rgba(236,238,251,0.78)', maxWidth: '50ch', marginTop: 'var(--s-4)' }}>
                <span className="lang-es" aria-hidden={lang !== 'es'}>Suscribite a nuestra lista de IR para recibir comunicados de prensa, resultados trimestrales y eventos relevantes en tu casilla.</span>
                <span className="lang-en" aria-hidden={lang !== 'en'}>Subscribe to our IR list to get press releases, quarterly results and material events in your inbox.</span>
              </p>
              <a
