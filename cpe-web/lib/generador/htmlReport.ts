...
export function generarReporteHTML(datos: DatosIngresos, macro?: MacroSnapshot, cclRate?: number | null): string {
  ...
  // Valorización: % oil (BOE≈bbl) × precio oil + % gas (convertido de BOE a
  // Mcf con el factor estándar de la industria, 6 Mcf = 1 BOE) × precio gas.
  // Se calcula dos veces con distinta base de volumen:
  //  - "vendida" usa el volumen VENDIDO (reconcilia con Ventas del Período,
  //    ya que el producido incluye stock que aún no se vendió)
  //  - "producida" usa el volumen PRODUCIDO (ingreso potencial si se vendiera
  //    toda la producción del período, sin stock)
  const BOE_TO_MCF = 6
  const valorizarDia = (volBoed: number, oilPct: number, gasPct: number) => {
    const oilVolBbld = volBoed * (oilPct / 100)
...
<div class="card-full">
  <div class="card-hdr">Saldo estimado al cierre del mes <span class="card-hdr-val">Volumen en m³</span></div>
  <p style="font-size:11px;color:var(--muted2);margin:-8px 0 14px">
    "Saldo estimado al cierre" es el mismo volumen que "Stock mes siguiente" en el detalle por área de arriba.
  </p>
  <table class="t">
    <thead><tr>
      <th>Área</th><th>Stock (m³)</th><th>Producción (m³)</th><th>Merma (m³)</th>
      <th>Entrega In Kind (m³)</th><th>Venta (m³)</th><th>Saldo estimado al cierre (m³)</th>
    </tr></thead>
    <tbody>
