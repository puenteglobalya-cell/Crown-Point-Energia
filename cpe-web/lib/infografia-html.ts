...
export type InfografiaData = {
  ...
  thesis?: { prodVal: string; prodUnit: string; prodDelta: string; resVal: string; resUnit: string; resDelta: string; ebVal: string; ebUnit: string; ebDelta: string }
  date: string
  url: string
  modules?: string[]
}
...
const ALL_MODULES = ['headline', 'stats', 'production', 'mix', 'blocks', 'thesis', 'stock', 'ratings']

export function generateInfografiaHtml(d: InfografiaData): string {
  const active = d.modules ?? ALL_MODULES
  ...
  const t = d.thesis
  const gasP = parseInt((d.production.mix.match(/^(\d+)/) ?? ['54'])[1])
  const liqP = 100 - gasP

  const thesisSection = has('thesis') && t ? `
  <!-- Thesis -->
  <div style="padding:26px 48px;border-bottom:1px solid rgba(108,174,82,0.22);background:rgba(15,27,56,0.4);">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);border:1px solid rgba(108,174,82,0.22);border-radius:10px;overflow:hidden;">
      ${[
        ...
        <div style="padding:18px 20px;${i < 3 ? 'border-right:1px solid rgba(108,174,82,0.18);' : ''}background:rgba(31,37,102,0.3);">
          ...
          <div>
            <span style="font-family:'JetBrains Mono',monospace;font-size:26px;font-weight:700;color:#fff;letter-spacing:-0.02em;">${k.val}</span>
            <span style="font-size:12px;color:rgba(255,255,255,0.45);margin-left:6px;">${k.unit}</span>
          </div>
          <div style="font-size:11px;color:#6CAE52;margin-top:4px;font-weight:600;">${k.delta}</div>
        </div>
      `).join('')}
    </div>
  </div>` : ''

  const mixSection = has('mix') ? `
  <!-- Mix -->
  <div style="padding:22px 48px;border-bottom:1px solid rgba(108,174,82,0.22);">
    <div style="font-size:10px;letter-spacing:0.2em;text-transform:uppercase;color:rgba(255,255,255,0.4);font-weight:700;margin-bottom:14px;">
      MIX DE PRODUCCIÓN
    </div>
    <div style="display:flex;gap:20px;align-items:center;">
      <div style="flex:1;height:28px;border-radius:999px;overflow:hidden;display:flex;">
        <div style="width:${gasP}%;background:#4a8a3a;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;">${gasP}% Gas</div>
        <div style="width:${liqP}%;background:#1F2566;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;">${liqP}% Líquidos</div>
      </div>
      <div style="font-size:12px;color:rgba(255,255,255,0.45);min-width:100px;">Q1 2026 · neto</div>
    </div>
  </div>` : ''

  const stockCol = has('stock') ? `
    <div style="padding:28px 48px;${has('ratings') ? 'border-right:1px solid rgba(108,174,82,0.22);' : ''}border-bottom:1px solid rgba(108,174,82,0.22);">
  ...
  const ratingsCol = has('ratings') ? `
    <div style="padding:28px 48px;border-bottom:1px solid rgba(108,174,82,0.22);">
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid rgba(108,174,82,0.2);">
        <div style="font-family:'JetBrains Mono',monospace;font-size:28px;font-weight:700;color:#031636;background:#6CAE52;padding:8px 18px;border-radius:8px;letter-spacing:-0.02em;">
  ...
  const stockRatingsSection = (has('stock') || has('ratings')) ? `
  <!-- Stock + Ratings -->
  <div style="display:grid;grid-template-columns:${has('stock') && has('ratings') ? '1fr 1fr' : '1fr'};flex:1;">
    ${stockCol}${ratingsCol}
  </div>` : ''
